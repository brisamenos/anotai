...
// ── Sessões Admin (persistidas no SQLite, válidas 8h) ─
...
async function handleIAWebhook(req, res) {
  ...
  try {
    ...
    if (pausaAt&&(Date.now()-pausaAt)<pausaMin*60*1000) { send(res,200,{ok:true}); return }
    ...
    const timer = setTimeout(async () => {
      ...
      if(numMatch){const n=parseInt(numMatch[1]);let ped=db.prepare("SELECT id,status,total,items,client,addr,pag FROM orders WHERE tenant_id=? AND id=?").get(tenantId,n)||db.prepare("SELECT id,status,total,items,client,addr,pag FROM orders WHERE id=? AND phone LIKE ?").get(n,`%${phone.slice(-8)}%`)||db.prepare("SELECT id,status,total,items,client,addr,pag FROM orders WHERE id=?").get(n);const sl={analise:'⏳ aguardando',producao:'👨‍🍳 em preparo',pronto:'✅ pronto',saiu:'🛵 saiu',entregue:'🎉 entregue',cancelado:'❌ cancelado',finalizado:'✅ finalizado'};if(ped){const its=(()=>{try{return(JSON.parse(ped.items)||[]).map(i=>`${i.qty}x ${i.name}`).join(', ')}catch{return''}})();contexto.push(`PEDIDO #${ped.id}:\n  Status: ${sl[ped.status]||ped.status}\n  Total: R$${parseFloat(ped.total).toFixed(2).replace('.',',')}\n  Itens: ${its}`)}else{contexto.push(`PEDIDO #${n}: não encontrado.`)}}
...
const server = http.createServer(async (req,res) => {
  ...
  if(req.method==='POST'&&upath==='/api/customer-register'){
    const body=await readBody(req),tid=getTenantId(req,params),{name,phone,email,senha,birthday}=body
    ...
    try{const hash=crypto.createHash('sha256').update(senha).digest('hex');const existing=db.prepare('SELECT id FROM customers WHERE tenant_id=? AND phone=?').get(tid,phone);if(existing){db.prepare('UPDATE customers SET name=?,email=?,birthday=?,senha_hash=? WHERE tenant_id=? AND phone=?').run(name,email||null,birthday||null,hash,tid,phone);const c=db.prepare('SELECT id,name,phone,email,birthday,orders_count,total_spent,created_at FROM customers WHERE tenant_id=? AND phone=?').get(tid,phone);send(res,200,{...c,token:Buffer.from(`${c.id}:${tid}:${hash.slice(0,16)}`).toString('base64')});return};const info=db.prepare('INSERT INTO customers (tenant_id,name,phone,email,birthday,senha_hash,orders_count,total_spent) VALUES (?,?,?,?,?,?,0,0)').run(tid,name,phone,email||null,birthday||null,hash);const c=db.prepare('SELECT id,name,phone,email,birthday,orders_count,total_spent,created_at FROM customers WHERE id=?').get(info.lastInsertRowid);marcarDirty();send(res,201,{...c,token:Buffer.from(`${c.id}:${tid}:${hash.slice(0,16)}`).toString('base64')})}catch(e){send(res,400,{error:e.message})}
    return
  }

  if(req.method==='POST'&&upath==='/api/customer-login'){
    const body=await readBody(req),tid=getTenantId(req,params),{phone,senha}=body
    if(!phone||!senha){send(res,400,{error:'Telefone e senha obrigatórios'});return}
    if(!tid){send(res,400,{error:'Tenant não identificado'});return}
    try{const hash=crypto.createHash('sha256').update(senha).digest('hex');const c=db.prepare('SELECT id,name,phone,email,birthday,orders_count,total_spent,created_at,senha_hash FROM customers WHERE tenant_id=? AND phone=?').get(tid,phone);if(!c||!c.senha_hash){send(res,401,{error:'Telefone não cadastrado'});return};if(c.senha_hash!==hash){send(res,401,{error:'Senha incorreta'});return};const{senha_hash:_,...safe}=c;send(res,200,{...safe,token:Buffer.from(`${c.id}:${tid}:${hash.slice(0,16)}`).toString('base64')})}catch(e){send(res,400,{error:e.message})}
    return
  }

  if(req.method==='GET'&&upath==='/api/customer-orders'){
    const tid=getTenantId(req,params),cid=params.get('customer_id')
    if(!tid||!cid){send(res,400,{error:'Parâmetros faltando'});return}
    try{const rows=db.prepare('SELECT id,client,phone,addr,items,total,taxa,pag,status,created_at FROM orders WHERE tenant_id=? AND customer_id=? ORDER BY id DESC LIMIT 30').all(tid,cid);send(res,200,rows.map(r=>({...r,items:(()=>{try{return JSON.parse(r.items)}catch{return[]}})()})))}catch(e){send(res,400,{error:e.message})}
    return
  }

  if(req.method==='POST'&&upath==='/api/criar-tenant'){
    const body=await readBody(req),{nome,plano,slug,email,senha,role,nomeGestor}=body
    if(!nome||!email||!senha){send(res,400,{error:'nome, email e senha obrigatórios'});return}
    try{const hash=crypto.createHash('sha256').update(senha).digest('hex');const slugBase=slug||nome.toLowerCase().replace(/[^a-z0-9]/g,'-').replace(/-+/g,'-').replace(/^-|-$/g,'');let slugFinal=slugBase,suffix=2;while(db.prepare('SELECT id FROM tenants WHERE slug=?').get(slugFinal))slugFinal=`${slugBase}-${suffix++}`;if(slug&&slugFinal!==slug){send(res,400,{error:`Slug "${slug}" já em uso. Sugerimos: "${slugFinal}"`});return};if(db.prepare('SELECT id FROM sys_users WHERE email=?').get(email)){send(res,400,{error:`E-mail "${email}" já cadastrado.`});return};db.prepare('INSERT INTO tenants (nome,plano,slug) VALUES (?,?,?)').run(nome,plano||'basic',slugFinal);const t=db.prepare('SELECT id FROM tenants WHERE slug=?').get(slugFinal);db.prepare('INSERT OR IGNORE INTO store_config (tenant_id) VALUES (?)').run(t.id);db.prepare('INSERT INTO sys_users (nome,email,senha_hash,role,tenant_id) VALUES (?,?,?,?,?)').run(nomeGestor||nome,email,hash,role||'gestor',t.id);marcarDirty();setTimeout(()=>fazerBackup(true),2000);send(res,201,{ok:true,tenant_id:t.id,slug:slugFinal})}catch(e){send(res,400,{error:e.message})}
    return
  }
  ...
  if(upath.startsWith('/api/evo')){
    ...
    const body=['POST','DELETE'].includes(req.method)?await readBody(req):{},action=upath.replace('/api/evo','')
    ...
    try{const r=await fetch(`${EVO_URL}${evoPath}`,{method:req.method,headers:{'Content-Type':'application/json',apikey:EVO_KEY},body:req.method!=='GET'?JSON.stringify(body):undefined});const data=await r.json().catch(()=>({}));if(evoPath.startsWith('/instance/create')&&r.ok&&body.instanceName)db.prepare('UPDATE store_config SET evo_instance=? WHERE tenant_id=?').run(body.instanceName,tenantId);send(res,r.status,data)}catch(e){send(res,500,{error:e.message})}
    return
  }

  if(req.method==='POST'&&upath==='/enviar'){const{phone,text,tenant_id}=await readBody(req);if(!phone||!text){send(res,400,{ok:false,error:'phone e text obrigatórios'});return};const tid=tenant_id||req.headers['x-tenant-id'];const cfgEnv=tid?db.prepare('SELECT evo_instance FROM store_config WHERE tenant_id=?').get(tid):null;const r=await sendWA(phone,text,cfgEnv?.evo_instance);send(res,r.ok?200:500,r);return}

  if(req.method==='POST'&&upath==='/promocao'){
    const body=await readBody(req),{destino='todos',msg,tenant_id}=body
    if(!msg){send(res,400,{ok:false,error:'msg obrigatório'});return}
    const tid=tenant_id||req.headers['x-tenant-id'],cfgP=tid?db.prepare('SELECT evo_instance FROM store_config WHERE tenant_id=?').get(tid):null,instP=cfgP?.evo_instance||EVO_INST
    ...
    ;(async()=>{let ok=0,fail=0;for(const c of cl){const r=await sendWA(c.phone,fillVars(msg,{nome:c.name}),instP);r.ok?ok++:fail++;await sleep(1500)};log('📢',`Promoção: ${ok} ok, ${fail} fail`)})()
    return
  }

  if(req.method==='POST'&&upath==='/aniversario'){checarAniv();send(res,200,{ok:true});return}
  ...
  if(req.method==='POST'&&upath==='/api/ia-humano-assumiu'){const{phone,tenant_id}=await readBody(req);if(phone&&tenant_id){_pausaHumano.set(`pausa:${tenant_id}:${phone}`,Date.now());log('👤',`Humano assumiu conversa com ${phone}`)};send(res,200,{ok:true});return}
