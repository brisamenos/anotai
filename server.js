...
async function handleIAWebhook(req, res) {
  ...
  try {
    ...
    const timer = setTimeout(async () => {
      ...
      const pedCli=db.prepare("SELECT id,order_num,status,total,items,client,addr,pag,created_at FROM orders WHERE tenant_id=? AND phone LIKE ? ORDER BY id DESC LIMIT 5").all(tenantId,`%${phone.slice(-8)}%`)
      if(pedCli.length){const sl={analise:'⏳ Em análise',producao:'👨‍🍳 Em preparo',pronto:'✅ Pronto para retirada',saiu:'🛵 Saiu para entrega',entregue:'🎉 Entregue',cancelado:'❌ Cancelado',finalizado:'✅ Finalizado'};contexto.push('PEDIDOS DESTE CLIENTE (número que o cliente conhece):\n'+pedCli.map(p=>{const its=(()=>{try{return(JSON.parse(p.items)||[]).map(i=>`${i.qty}x ${i.name}`).join(', ')}catch{return''}})();return`  Pedido #${_iaPedNum(p)}: ${sl[p.status]||p.status} — R$${parseFloat(p.total).toFixed(2).replace('.',',')}${its?' — Itens: '+its:''}`}).join('\n'))}
      const numMatch=msgFull.match(/#\*?(\d{1,6})\*?/)||msgFull.match(/pedido\s*[*#]?\s*(\d{1,6})/i)
      if(numMatch){const n=parseInt(numMatch[1]);const realId=n+_iaOffset;let ped=db.prepare("SELECT id,order_num,status,total,items,client,addr,pag,created_at FROM orders WHERE tenant_id=? AND id=?").get(tenantId,realId);if(!ped)ped=db.prepare("SELECT id,order_num,status,total,items,client,addr,pag,created_at FROM orders WHERE tenant_id=? AND order_num=?").get(tenantId,n);if(!ped)ped=db.prepare("SELECT id,order_num,status,total,items,client,addr,pag,created_at FROM orders WHERE tenant_id=? AND phone LIKE ? ORDER BY id DESC LIMIT 1").get(tenantId,`%${phone.slice(-8)}%`);const sl={analise:'⏳ Em análise — seu pedido foi recebido e está aguardando confirmação',producao:'👨‍🍳 Em preparo — estamos preparando seu pedido agora',pronto:'✅ Pronto — seu pedido está pronto para retirada/entrega',saiu:'🛵 Saiu para entrega — o entregador está a caminho',entregue:'🎉 Entregue — pedido finalizado',cancelado:'❌ Cancelado',finalizado:'✅ Finalizado'};if(ped){const pedNum=_iaPedNum(ped);const its=(()=>{try{return(JSON.parse(ped.items)||[]).map(i=>`${i.qty}x ${i.name}`).join(', ')}catch{return''}})();contexto.push(`DETALHES DO PEDIDO #${pedNum} (este é o número correto do pedido):\n  Status: ${sl[ped.status]||ped.status}\n  Cliente: ${ped.client||'N/A'}\n  Total: R$${parseFloat(ped.total).toFixed(2).replace('.',',')}\n  Itens: ${its||'N/A'}\n  Endereço: ${ped.addr||'Retirada/Mesa'}\n  Pagamento: ${ped.pag||'N/A'}\n\nIMPORTANTE: O número do pedido deste cliente é #${pedNum}. Use APENAS este número ao se referir ao pedido.`)}else{contexto.push(`PEDIDO #${String(n).padStart(3,'0')}: não encontrado no sistema. Peça ao cliente para confirmar o número.`)}}
