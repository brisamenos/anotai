...
// ═══════════════════════════════════════════════════════
// WHATSAPP CHAT — Gestor de Conversas (Evolution API)
// ═══════════════════════════════════════════════════════
...
const WA = {
  ...
  filteredChats: [],   // chats após filtro de busca
  activeJid: null,     // JID da conversa ativa
  activeName: '',
  activePhone: '',
  messages: [],        // mensagens da conversa ativa
  avatarCache: {},     // jid → url da foto de perfil
  pendingFile: null,   // arquivo selecionado para enviar
  mediaRecorder: null,
  audioChunks: [],
  recTimer: null,
  recSeconds: 0,
  pollTimer: null,     // timer de polling de novas msgs
  lastMsgId: null,
  ...
  // Extrai número limpo para envio
  cleanNumber(jid) {
  ...
  async checkConnection() {
    const inst = (document.getElementById('evo-instance')?.value||'').trim();
    const statusEl = document.getElementById('wa-conn-status');
    if (!inst) {
      if (statusEl) statusEl.textContent = '⚠️ Configure instância no Robô';
      return false;
    }
    try {
      ...
      if (statusEl) {
        statusEl.textContent = connected
          ? '🟢 Conectado'
          : `⚠️ ${state || 'Desconectado'} — conecte no Robô`;
      }
      return connected;
    } catch(e) {
      if (statusEl) statusEl.textContent = '⚠️ Erro ao verificar conexão';
      return false;
    }
  },
  ...
  async fetchAvatar(jid) {
    ...
    this.avatarCache[jid] = null; // marca como buscado
    try {
  ...
  // ── Renderizar avatar num elemento ───────────────────
  async renderAvatar(jid, name, el) {
    const url = await this.fetchAvatar(jid);
    if (url && el) {
      el.innerHTML = `<img src="${url}" onerror="this.parentElement.textContent='${this.initials(name)}'" alt="">`;
    } else if (el) {
...
function waOpenPanel() {
  ...
  closeNotif?.();
  // Verifica conexão e carrega chats
  WA.checkConnection().then(connected => {
    if (connected) waLoadChats();
  });
  // Polling de novas mensagens na conversa ativa
  if (!WA.pollTimer) {
...
function waBackToList() {
  const listCol = document.getElementById('wa-list-col');
  if (listCol) listCol.classList.remove('hidden');
}
...
async function waLoadChats() {
  ...
  if (!inst) {
    listEl.innerHTML = `<div class="wa-empty-state"><div style="font-size:13px;color:var(--muted)">⚠️ Configure a instância no painel Robô primeiro.</div></div>`;
    return;
  }
  ...
  try {
    ...
    if (!Array.isArray(chats)) {
      if (Array.isArray(chats?.chats)) chats = chats.chats;
      else if (Array.isArray(chats?.data)) chats = chats.data;
      else chats = [];
    }
    ...
    chats.sort((a, b) => {
      const ta = a.lastMessageTimestamp || a.updatedAt || 0;
      const tb = b.lastMessageTimestamp || b.updatedAt || 0;
      return (typeof tb === 'number' ? tb : new Date(tb||0).getTime()/1000)
           - (typeof ta === 'number' ? ta : new Date(ta||0).getTime()/1000);
    });
    ...
    listEl.innerHTML = `<div class="wa-empty-state"><div style="font-size:12px;color:var(--danger)">Erro ao carregar conversas: ${e.message}</div><button class="wa-btn-primary" style="margin-top:12px;font-size:12px;padding:7px 18px" onclick="waLoadChats()">Tentar novamente</button></div>`;
  }
}
...
function waRenderChatList(chats) {
  ...
  if (!chats.length) {
    listEl.innerHTML = `<div class="wa-empty-state"><div style="font-size:13px;color:var(--muted)">Nenhuma conversa encontrada.<br>Inicie uma conversa pelo WhatsApp.</div><button class="wa-btn-primary" style="margin-top:12px;font-size:12px;padding:7px 18px" onclick="waLoadChats()">Atualizar</button></div>`;
    return;
  }
  ...
  listEl.innerHTML = chats.map((c,i) => {
    ...
    const ts      = c.lastMessageTimestamp || c.updatedAt;
    const timeStr = WA.formatTime(ts);
    const preview = c.lastMessage?.conversation || c.lastMessage?.message?.conversation
                 || c.lastMessage?.extendedTextMessage?.text
                 || (c.lastMessage?.message ? '📎 Mídia' : '');
    const unread  = c.unreadCount || 0;
    const initials= WA.initials(name);
    const isActive= jid === WA.activeJid;

    return `<div class="wa-chat-item${isActive?' active':''}" onclick="waOpenConversation('${jid.replace(/'/g,"\\'")}','${name.replace(/'/g,"\\'")}')" id="wa-ci-${i}">
      <div class="wa-chat-avatar" id="wa-av-${i}">${initials}</div>
      <div class="wa-chat-meta">
        <div class="wa-chat-name">${waEsc(name)}</div>
        <div class="wa-chat-preview">${waEsc(preview) || '<span style="font-style:italic;opacity:.6">Sem mensagens</span>'}</div>
      </div>
      ${timeStr ? `<div class="wa-chat-time">${timeStr}</div>` : ''}
      ${unread > 0 ? `<div class="wa-unread-dot">${unread}</div>` : ''}
    </div>`;
  }).join('');

  // Carrega avatares de forma assíncrona
  chats.forEach((c, i) => {
    const jid = c.id || c.remoteJid || '';
    const name = c.name || c.pushName || WA.formatPhone(jid);
    const el = document.getElementById(`wa-av-${i}`);
    if (el) WA.renderAvatar(jid, name, el);
  });
}
...
async function waOpenConversation(jid, name) {
  WA.activeJid = jid;
  WA.activeName = name;
  WA.activePhone = WA.formatPhone(jid);
  WA.lastMsgId = null;
  WA.messages = [];
  ...
  const emptyEl  = document.getElementById('wa-conv-empty');
  const activeEl = document.getElementById('wa-conv-active');
  ...
  // Carrega avatar
  if (avatarEl) WA.renderAvatar(jid, name, avatarEl);

  // Mobile: esconde lista
  const listCol = document.getElementById('wa-list-col');
  if (listCol && window.innerWidth <= 640) listCol.classList.add('hidden');
...
async function waLoadMessages(silent = false) {
  ...
  try {
    ...
    if (!Array.isArray(msgs)) {
      if (Array.isArray(msgs?.messages)) msgs = msgs.messages;
      else if (Array.isArray(msgs?.data)) msgs = msgs.data;
      else msgs = [];
    }
    ...
    // Verifica se mudou (polling silencioso)
    if (silent && msgs.length > 0) {
      const lastId = msgs[msgs.length-1].key?.id;
      if (lastId === WA.lastMsgId) return; // nada novo
      WA.lastMsgId = lastId;
    } else if (msgs.length > 0) {
      WA.lastMsgId = msgs[msgs.length-1].key?.id;
    }
    ...
    // Render messages com separadores de data
    let lastDate = '';
    msgs.forEach(msg => {
    ...
    if (loadEl) loadEl.style.display = 'none';
    if (!silent) {
      msgsEl.innerHTML = `<div style="text-align:center;color:var(--danger);font-size:12px;padding:20px">Erro ao carregar mensagens: ${e.message}</div>`;
    }
  }
}
...
function waCreateMsgEl(msg) {
  ...
  const div = document.createElement('div');
  div.className = `wa-msg ${isFromMe ? 'sent' : 'recv'}`;
  ...
  if (imgMsg) {
    ...
    const b64 = msg._base64 || imgMsg._base64 || null;
    if (b64) {
      bubble.innerHTML = `<img src="data:${imgMsg.mimetype||'image/jpeg'};base64,${b64}" alt="Imagem" onclick="waViewMedia(this.src,'image')" loading="lazy">${caption?`<div style="margin-top:4px;font-size:13px">${waEsc(caption)}</div>`:''}`;
    } else {
      bubble.innerHTML = `<div class="wa-doc-bubble"><span class="wa-doc-icon">🖼️</span><span style="font-size:12.5px">Imagem${caption?': '+waEsc(caption):''}</span></div>`;
    }
  }
  // Vídeo
  else if (vidMsg) {
    const caption = vidMsg.caption || '';
    const b64 = msg._base64 || null;
    if (b64) {
      bubble.innerHTML = `<video src="data:${vidMsg.mimetype||'video/mp4'};base64,${b64}" controls style="max-width:240px;max-height:200px;border-radius:8px"></video>${caption?`<div>${waEsc(caption)}</div>`:''}`;
    } else {
      bubble.innerHTML = `<div class="wa-doc-bubble"><span class="wa-doc-icon">🎥</span><span style="font-size:12.5px">Vídeo${caption?': '+waEsc(caption):''}</span></div>`;
    }
  }
  // Áudio
  else if (audioMsg) {
    const b64 = msg._base64 || null;
    if (b64) {
      bubble.innerHTML = `<div class="wa-audio-player"><audio controls src="data:${audioMsg.mimetype||'audio/ogg'};base64,${b64}"></audio></div>`;
    } else {
      bubble.innerHTML = `<div class="wa-audio-player"><span style="font-size:20px">🎵</span><span style="font-size:12px;color:var(--muted)">Áudio (baixe no app)</span></div>`;
    }
  }
  // Documento
  else if (docMsg) {
    ...
    bubble.innerHTML = `<div class="wa-doc-bubble"><span class="wa-doc-icon">${icon}</span><span style="font-size:12.5px">${waEsc(fname)}</span></div>`;
  }
  // Sticker
  else if (stickerMsg) {
    bubble.innerHTML = `<span style="font-size:13px;color:var(--muted)">🎭 Figurinha</span>`;
  }
  // Texto
  else if (text) {
  ...
  timeDiv.innerHTML = `${timeStr}${isFromMe ? ' <span class="wa-check">✓✓</span>' : ''}`;
...
// ── Visualizar imagem em fullscreen ──────────────────
function waViewMedia(src, type) {
  const ov = document.createElement('div');
  ov.style.cssText = 'position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,.9);display:flex;align-items:center;justify-content:center;cursor:zoom-out';
...
async function waSendMessage() {
  ...
  if (!inst || !WA.activeJid) return;

  // Se há arquivo pendente, envia mídia
  if (WA.pendingFile) {
    await waSendMedia();
    return;
  }
  ...
  inp.value = '';
  waToggleSendMic(false); // volta para microfone
  ...
  const el = waCreateMsgEl(tempMsg);
  const msgsEl = document.getElementById('wa-messages');
  if (el && msgsEl) { msgsEl.appendChild(el); msgsEl.scrollTop = msgsEl.scrollHeight; }

  try {
    const r = await EVO.sendText(WA.cleanNumber(WA.activeJid), text);
    if (!r.ok) { sbToast('err', 'Erro ao enviar mensagem'); }
    // Reload em 1.5s para confirmar entrega
    setTimeout(() => waLoadMessages(true), 1500);
  } catch(e) {
    sbToast('err', 'Erro ao enviar: ' + e.message);
  }
}

// ── Tipagem → toggle ícone send/mic ──────────────────
...
function waOnFileSelect(event) {
  const file = event.target.files?.[0];
  if (!file) return;
  WA.pendingFile = file;

  const previewEl = document.getElementById('wa-media-preview');
  const previewImg= document.getElementById('wa-preview-img');
  const previewNm = document.getElementById('wa-preview-name');
  if (!previewEl) return;

  previewEl.style.display = 'block';
  if (previewNm) previewNm.textContent = file.name;
  ...
  reader.onload = async (e) => {
    ...
    try {
      const r = await EVO.req('POST', `/message/sendMedia/${EVO.instance}`, {
        number:    WA.cleanNumber(WA.activeJid),
        mediatype,
        mimetype:  mime,
        caption,
        media:     b64,
      ...
      if (r.ok) {
        sbToast('ok', 'Mídia enviada!');
        setTimeout(() => waLoadMessages(true), 1500);
      } else {
        sbToast('err', 'Erro ao enviar mídia');
      }
    } catch(err) {
      sbToast('err', 'Erro: ' + err.message);
    }
  };
  reader.readAsDataURL(file);
}
...
async function waStartAudio() {
  ...
  try {
    ...
    WA.audioChunks = [];
    WA.recSeconds  = 0;
    WA.mediaRecorder = new MediaRecorder(stream, { mimeType: MediaRecorder.isTypeSupported('audio/webm;codecs=opus') ? 'audio/webm;codecs=opus' : 'audio/webm' });
    WA.mediaRecorder.ondataavailable = e => { if (e.data?.size > 0) WA.audioChunks.push(e.data); };
    WA.mediaRecorder.start(100);

    // UI de gravação
    const recEl = document.getElementById('wa-audio-recording');
    const micBtn = document.getElementById('wa-mic-btn');
    ...
    WA.recTimer = setInterval(() => {
      WA.recSeconds++;
      const timeEl = document.getElementById('wa-rec-time');
      if (timeEl) timeEl.textContent = `${Math.floor(WA.recSeconds/60)}:${String(WA.recSeconds%60).padStart(2,'0')}`;
    }, 1000);
  } catch(e) {
    sbToast('err', 'Não foi possível acessar o microfone');
  }
}

async function waStopAudio() {
  if (!WA.mediaRecorder) return;
  clearInterval(WA.recTimer);
  WA.recTimer = null;
  ...
  const chunks = [...WA.audioChunks];
  WA.audioChunks = [];

  if (!chunks.length || WA.recSeconds < 1) return; // muito curto
  ...
  reader.onload = async (e) => {
    ...
    try {
      ...
      if (r.ok) {
        ...
        sbToast('err', 'Erro ao enviar áudio');
      }
    } catch(err) {
      sbToast('err', 'Erro: ' + err.message);
    }
...
// ── Inicialização: esconde botão send, mostra mic ─────
(function waInit() {
  // Aguarda DOM
  const tryInit = () => {
    const sendBtn = document.getElementById('wa-send-btn');
