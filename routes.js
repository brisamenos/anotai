...
module.exports = async function handleRoutes(req, res, ctx) {
  ...
  // ══════════════════════════════════════════════════════
  // CHAT DE SUPORTE — Cliente → Gestor
  // ══════════════════════════════════════════════════════

  // ── Cliente envia mensagem ──────────────────────────
  if (req.method === 'POST' && upath === '/api/chat/enviar') {
    const tid = req.headers['x-tenant-id']
    if (!tid) { send(res, 400, { error: 'x-tenant-id obrigatório' }); return true }
    const body = await readBody(req)
    const { message, customer_name, customer_phone, customer_id } = body
    if (!message?.trim()) { send(res, 400, { error: 'Mensagem vazia' }); return true }
    if (!customer_name && !customer_phone) { send(res, 400, { error: 'Identificação do cliente obrigatória' }); return true }
    try {
      const info = db.prepare(`INSERT INTO chat_messages
        (tenant_id, customer_id, customer_name, customer_phone, sender, message, read_by_gestor)
        VALUES (?,?,?,?,?,?,0)`)
        .run(tid, customer_id||null, customer_name||'', customer_phone||'', 'client', message.trim())
      const row = db.prepare('SELECT * FROM chat_messages WHERE id=?').get(info.lastInsertRowid)
      sseBroadcast(`chat-gestor:${tid}`, 'chat_messages:INSERT', row)
      marcarDirty()
      send(res, 200, { ok: true, message: row })
    } catch(e) { send(res, 500, { error: e.message }) }
    return true
  }

  // ── Gestor responde mensagem ────────────────────────
  if (req.method === 'POST' && upath === '/api/chat/responder') {
    const tid = req.headers['x-tenant-id']
    if (!tid) { send(res, 400, { error: 'x-tenant-id obrigatório' }); return true }
    const body = await readBody(req)
    const { message, customer_phone } = body
    if (!message?.trim()) { send(res, 400, { error: 'Mensagem vazia' }); return true }
    if (!customer_phone)  { send(res, 400, { error: 'customer_phone obrigatório' }); return true }
    // Pega nome/id do cliente pela conversa
    const last = db.prepare('SELECT customer_name, customer_id FROM chat_messages WHERE tenant_id=? AND customer_phone=? ORDER BY id DESC LIMIT 1').get(tid, customer_phone)
    try {
      const info = db.prepare(`INSERT INTO chat_messages
        (tenant_id, customer_id, customer_name, customer_phone, sender, message, read_by_gestor)
        VALUES (?,?,?,?,?,?,1)`)
        .run(tid, last?.customer_id||null, last?.customer_name||'', customer_phone, 'gestor', message.trim())
      const row = db.prepare('SELECT * FROM chat_messages WHERE id=?').get(info.lastInsertRowid)
      // Envia para o cliente via SSE canal dele
      sseBroadcast(`chat-rt:${tid}`, 'chat_messages:INSERT', row)
      marcarDirty()
      send(res, 200, { ok: true, message: row })
    } catch(e) { send(res, 500, { error: e.message }) }
    return true
  ...
  if (req.method === 'GET' && upath === '/api/chat/historico') {
    const tid   = req.headers['x-tenant-id']
    const phone = params.get('phone') || ''
    if (!tid) { send(res, 400, { error: 'x-tenant-id obrigatório' }); return true }
    try {
      const msgs = phone
        ? db.prepare('SELECT * FROM chat_messages WHERE tenant_id=? AND customer_phone=? ORDER BY id ASC').all(tid, phone)
        : db.prepare('SELECT * FROM chat_messages WHERE tenant_id=? ORDER BY id ASC').all(tid)
      send(res, 200, msgs)
    } catch(e) { send(res, 500, { error: e.message }) }
    return true
  }

  // ── Lista conversas do gestor (uma por cliente) ────
  if (req.method === 'GET' && upath === '/api/chat/conversas') {
    const tid = req.headers['x-tenant-id']
    if (!tid) { send(res, 400, { error: 'x-tenant-id obrigatório' }); return true }
    try {
      // Última mensagem por telefone + count não lidas
      const convs = db.prepare(`
        SELECT customer_phone, customer_name, customer_id,
               ...
               (SELECT sender FROM chat_messages m3
                WHERE m3.tenant_id=m.tenant_id AND m3.customer_phone=m.customer_phone
                ORDER BY m3.id DESC LIMIT 1) as last_sender,
               SUM(CASE WHEN read_by_gestor=0 AND sender='client' THEN 1 ELSE 0 END) as unread
  ...
  if (req.method === 'POST' && upath === '/api/chat/ler') {
    const tid   = req.headers['x-tenant-id']
    const body  = await readBody(req)
    const phone = body.customer_phone || ''
    if (!tid || !phone) { send(res, 400, { error: 'tenant + phone obrigatórios' }); return true }
    try {
      db.prepare("UPDATE chat_messages SET read_by_gestor=1 WHERE tenant_id=? AND customer_phone=? AND sender='client'").run(tid, phone)
      send(res, 200, { ok: true })
    } catch(e) { send(res, 500, { error: e.message }) }
    return true
  }
