...
const customers = db.prepare(`SELECT c.id, c.name, c.phone, c.email, c.birthday, c.addr, c.created_at, COUNT(o.id)                       AS orders_count, COALESCE(SUM(o.total + o.taxa), 0) AS total_spent, MAX(o.created_at)                  AS last_order_at FROM customers c LEFT JOIN orders o ON o.tenant_id = c.tenant_id AND (o.customer_id = c.id OR o.phone = c.phone) AND o.status NOT IN ('cancelado', 'aguardando_pix') WHERE c.tenant_id = ? GROUP BY c.id ORDER BY c.created_at DESC`).all(tid)
...
`, `*Nome:* ${nome}`, `*Restaurante:* ${restaurante || '—'}`, `*Telefone:* ${telefone}`, `*Cidade:* ${cidade || '—'}`, `*Plano de interesse:* ${planoLabel}`, `,
...
try {
db.prepare(`CREATE TABLE IF NOT EXISTS leads_trial ( id INTEGER PRIMARY KEY AUTOINCREMENT, nome TEXT, restaurante TEXT, telefone TEXT, cidade TEXT, plano TEXT, created_at TEXT DEFAULT (datetime('now')) )`).run()
...
try {
db.exec(`CREATE TABLE IF NOT EXISTS print_jobs ( id INTEGER PRIMARY KEY AUTOINCREMENT, tenant_id TEXT NOT NULL, html TEXT NOT NULL, format TEXT DEFAULT 'A4', printer TEXT, status TEXT DEFAULT 'pending', error TEXT, created_at TEXT DEFAULT (datetime('now')), done_at TEXT )`)
...
```
    ...
    if (fmt === '80mm' || fmt === '58mm') {
      // Define viewport com a largura real do papel (em px a 96dpi) antes de
      // carregar o conteúdo, para que scrollHeight reflita o tamanho correto
      // ao renderizar na largura da impressora (e não no viewport padrão de 800px).
      const paperPx = fmt === '58mm' ? 220 : 302
      await page.setViewport({ width: paperPx, height: 1200 })
    }
