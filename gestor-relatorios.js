...
return `<div class="kds-item2" style="display:flex;align-items:center;justify-content:space-between;gap:8px"> <div style="display:flex;align-items:center;gap:6px;flex:1"> <span class="kds-item2-qty">${item.qty}×</span> <div> <div class="kds-item2-name">${item.name.toUpperCase()}</div> ${item.obs ? `<div class="kds-item2-obs">${item.obs}</div>` : ''} </div> </div> <button onclick="kdsItemPronto(${o.id},'${itemId}')" style="padding:4px 10px;border-radius:7px;border:none;background:rgba(34,197,94,.15);color:var(--success);font-size:11px;font-weight:700;cursor:pointer;white-space:nowrap"> ✅ Pronto </button> </div>`;
}).join(’’);
return `<div class="kds-card2 ${cardCls}" id="kds-card-${o.id}"> <div class="kds-card2-head"> <div style="display:flex;align-items:center;gap:8px"> <div class="kds-card2-id">#${_orderNum(o.id, o.order_num)}</div> <span class="kds-card2-type kds-type-mesa">${o.addr || 'Mesa ' + o.mesa_num}</span> </div> <div class="kds-timer ${timerCls}" id="kds-timer-${o.id}">${_kdsFormatTime(elapsed)}</div> </div> <div class="kds-card2-body">${itemsHtml}</div> </div>`;
}).join(’’);
...
list.innerHTML = `<div style="background:var(--surface);border:1px solid var(--border);border-radius:12px;overflow:hidden"> <table style="width:100%;border-collapse:collapse"> <thead> <tr style="background:var(--surface2)"> <th style="padding:10px 14px;text-align:left;font-size:10.5px;font-weight:600;color:var(--muted);letter-spacing:.5px;text-transform:uppercase">Ingrediente</th> <th style="padding:10px 14px;text-align:center;font-size:10.5px;font-weight:600;color:var(--muted);letter-spacing:.5px;text-transform:uppercase">Qty / Mín</th> <th style="padding:10px 14px;text-align:center;font-size:10.5px;font-weight:600;color:var(--muted);letter-spacing:.5px;text-transform:uppercase">Estoque</th> <th style="padding:10px 14px;text-align:right;font-size:10.5px;font-weight:600;color:var(--muted);letter-spacing:.5px;text-transform:uppercase">Custo unit.</th> <th style="padding:10px 14px;text-align:right;font-size:10.5px;font-weight:600;color:var(--muted);letter-spacing:.5px;text-transform:uppercase">Valor total</th> <th style="padding:10px 6px"></th> </tr> </thead> <tbody> ${filtered.map(e => { const pct = e.min_qty > 0 ? Math.min(100, (e.qty / (e.min_qty * 3)) * 100) : (e.qty > 0 ? 100 : 0); const isBaixo = e.qty <= e.min_qty; const isZero  = e.qty === 0; const barColor = isZero ? 'var(--danger)' : isBaixo ? 'var(--accent3)' : 'var(--success)'; const valorTotal = (e.qty * (e.custo||0)).toFixed(2).replace('.',','); return`<tr style="border-top:1px solid var(--border);transition:background .13s" onmouseenter="this.style.background='rgba(255,255,255,.02)'" onmouseleave="this.style.background=''">
...
return `<div style="margin-bottom:4px;word-break:break-word;overflow-wrap:break-word"> <div style="font-weight:bold">${nameRaw}</div> <div style="text-align:right;font-size:0.9em">${price}</div> ${obs} </div>`;
}
return `<div style="margin-bottom:3px"> <div style="display:flex;justify-content:space-between;gap:4px"> <span style="word-break:break-word;flex:1">${nameRaw}</span> <span style="white-space:nowrap;flex-shrink:0">${price}</span> </div>${obs} </div>`;
};
...
? `${taxa > 0 ? `<div>Subtotal…..${money(subtotal)}</div><div>Taxa……..${money(taxa)}</div>` : ''} <div style="font-weight:bold;font-size:1.1em">TOTAL ${money(total)}</div>`
: `${taxa > 0 ? `<div style="display:flex;justify-content:space-between"><span>Subtotal</span><span>${money(subtotal)}</span></div><div style="display:flex;justify-content:space-between"><span>Taxa entrega</span><span>${money(taxa)}</span></div>` : ''} <div style="display:flex;justify-content:space-between;font-weight:bold"><span>TOTAL</span><span>${money(total)}</span></div>`;

// ── Via Principal ──────────────────────────────────────────────
const viaPrincipal = `<div class="print-ticket" style="font-size:${fs}px;max-width:${is58 ? '48' : '72'}mm;overflow:hidden"> <div class="pt-center pt-large">${cfg.nome}</div> ${cfg.sub ? `<div class="pt-center" style="font-size:0.85em">${cfg.sub}</div>` : ‘’}
<hr class="pt-hr">
